## Example P2WPKH
This section will cover on how to use the Bitcoin SDK for building a transaction using P2WPKH address.
Suppose you already know what every line means, so in this section there will be no comment explaining every single line.

### P2WPKH PSBT Example
```js copy
import { Address, P2wpkhUtxo, BElectrsAPI, PSBT } from east-bitcoin-lib
...
const main = async () => {
    ...
    const p = new PSBT({
        ...
        inputs: [
            {
                utxo: await P2wpkhUtxo.fromBitcoinUtxo(utxo), // parse bitcoin utxo into P2wpkh utxo type
                value: utxo.value
            }
        ],
        ouputs: [
...
### P2WPKH PSBT Example with Custom Script Output
```js copy
import { Address, P2wpkhUtxo, BElectrsAPI, PSBT, OpReturn, Script } from east-bitcoin-lib
...
Go to the next page to see full example on how to build a transaction with PSBT for P2TR address input